...
def funcMarket(itemName, argument, townName):
    ...
    # 引数による販売品・注文品の分岐
    if argument == "-n":
        summary = f'{jsonTime.strftime("%H{0}%M{1}").format("時", "分")}現在の{itemName}の{townstr}販売状況は以下の通りです。\n{saleStr}\n\n時間経過により市場がこの通りでない可能性があります。'
    elif argument == "-r":
        summary = f'{jsonTime.strftime("%H{0}%M{1}").format("時", "分")}現在の{itemName}の{townstr}注文状況は以下の通りです。\n{reqStr}\n\n時間経過により市場がこの通りでない可能性があります。'
